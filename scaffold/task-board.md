...
| UI-1 | QA/Chat 界面美化 | C-视觉 | +1 | Markdown 渲染 + 头像 + 卡片式布局 + 发送按钮重设计 | ✅ 完成 @05-10 |
...
| T-21 | docker-compose 启动验证 | E-部署配置 | +1.5 | 验证 docker-compose up --build 能起两个容器，端口映射正确 | ✅ 完成 @05-10 |
...
| 任务ID | 描述 | 状态 | 产出文件 |
|--------|------|------|----------|
| T-DEPLOY-1 | docker-compose health check | ✅ 完成 @05-10 | docker-compose.yml + Dockerfile.backend（安装 curl） |
...
| 任务ID | 描述 | 状态 | 产出文件 |
|--------|------|------|----------|
| T-BENCH-1 | benchmark 数据（rag_benchmark.json，5 题 + baseline） | ✅ 完成 @05-10 | backend/benchmark/rag_benchmark.json |
| T-BENCH-2 | api-contract.md 引用 benchmark | ✅ 完成 @05-10 | docs/api-contract.md（RAG Benchmark 节） |
| T-BENCH-3 | Agent架构说明.md 引用 benchmark 结果 | ✅ 完成 @05-10 | docs/Agent架构说明.md（RAG Pipeline 节） |
