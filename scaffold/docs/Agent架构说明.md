...
**Benchmark 评估：**
基于自建医学知识问答 benchmark（5 题，覆盖 easy/medium/hard），当前系统 RAG pipeline 表现：

| 指标 | 平均值 | 说明 |
|------|--------|------|
| hit@5 | **0.78** | top-5 检索命中期望教材的比例 |
| citation_recall | **0.77** | 回答引用与期望教材的匹配率 |
| faithfulness | **0.89** | 回答与检索片段的一致性 |

> 数据来源：`backend/benchmark/rag_benchmark.json`，基于当前系统实测。
